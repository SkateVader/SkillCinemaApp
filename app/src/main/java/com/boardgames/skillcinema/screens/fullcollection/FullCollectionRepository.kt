...
class FullCollectionRepository @Inject constructor(
    ...
    private suspend fun fetchAllPages(fetchPage: suspend (Int) -> List<Movie>): List<Movie> {
        ...
        while (true) {
            try {
                val movies = fetchPage(page).filter { it.isCompleteData() && it.id != 0 }
    ...
    suspend fun getMoviesForCollection(collectionType: String): List<Movie> {
        return when (collectionType) {
            "TOP_250_BEST_FILMS" -> {
                ...
                fetchAllPages { page ->
                    api.getTopMovies(type = "TOP_250_BEST_FILMS", page = page).movies ?: emptyList()
                }
            }
            "TOP_100_POPULAR_FILMS" -> {
                // Загружаем все страницы для популярного.
                fetchAllPages { page ->
                    api.getPopularMovies(type = "TOP_100_POPULAR_FILMS", page = page).movies ?: emptyList()
                }
            }
            "PREMIERES" -> {
            ...
            "ACTION" -> {
                // Загружаем все страницы для фильмов с жанром "Боевик" (genres = "3") снятых в США (countries = "1")
                fetchAllPages { page ->
                    api.searchMovies(
                        keyword = "",
                        countries = "1",
            ...
            "DRAMA" -> {
                // Загружаем все страницы для фильмов с жанром "Драма" снятых во Франции (countries = "2", genres = "5")
                fetchAllPages { page ->
                    api.searchMovies(
                        keyword = "",
                        countries = "2",
